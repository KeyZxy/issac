...
public class PlayerManager : MonoBehaviour
{
    ...
    [Header("Player Stat")]
    public int playerHp = 10000; // 현재 체력
    public int playerMaxHp = 10000; // 최대 체력
    public float playerMoveSpeed = 5f; // 이동 속도
    public float playerTearSpeed = 5f; // 투사체 속도
    public float playerShotDelay = 0.5f; // 공격 딜레이
    public float playerDamage = 10f; // 데미지
    public float playerRange = 50f; // 사거리
    ...
    public void CheckedStatus()
    {
        ...
        {
            //Guppy's Tail 먹었을 때
            if (ItemManager.instance.PassiveItems[15]) // Guppy's tail 먹었을 때
            {
    ...
    public void GetDamage()
    {
        ...
        if (ItemManager.instance.PassiveItems[15] && CanGetDamage && CanBlockDamage == 0) // Guppy's tail 먹었을 때
        {
            playerHp--;
            CanGetDamage = false;
            if (playerHp > 0)
        ...
        if (ItemManager.instance.PassiveItems[14] && CanGetDamage && CanBlockDamage == 0) // Dead Cat 먹었을 때
        {
            playerHp--;
            CanGetDamage = false;
            if (playerHp > 0)
